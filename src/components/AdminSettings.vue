...
<template>
	<div>
		...
		<NcSettingsSection v-if="globalLibraryTemplatesSupported" :name="t('whiteboard', 'Organization library presets')">
			...
			<ul v-else class="library-template-list">
				<li v-for="template in globalLibraryTemplates"
					...
					<NcButton type="tertiary"
						:aria-label="t('whiteboard', 'Delete library preset {name}', { name: template.templateName })"
...
export default {
	...
	data() {
		return {
			...
			globalLibraryTemplatesSupported: loadState('whiteboard', 'globalLibraryTemplatesSupported', false),
	...
	mounted() {
		...
		if (this.globalLibraryTemplatesSupported) {
	...
	methods: {
		...
		async fetchGlobalLibraryTemplates() {
			...
			try {
				...
				showError(this.getErrorMessage(error, t('whiteboard', 'Failed to load organization library presets.')))
			} finally {
		...
		async uploadGlobalLibraryTemplate(event) {
			...
			try {
				...
				if (template?.templateName && Number.isFinite(template?.itemCount)) {
					...
					showSuccess(t('whiteboard', 'Organization library preset uploaded.'))
				}
				await this.fetchGlobalLibraryTemplates()
			} catch (error) {
				showError(this.getErrorMessage(error, t('whiteboard', 'Failed to upload organization library preset.')))
			} finally {
		...
		async deleteGlobalLibraryTemplate(templateName) {
			if (!window.confirm(t('whiteboard', 'Delete "{name}"? Existing whiteboards created from this preset keep their copied library items.', { name: templateName }))) {
				return
			}
			...
			try {
				...
				showSuccess(t('whiteboard', 'Organization library preset deleted.'))
				await this.fetchGlobalLibraryTemplates()
			} catch (error) {
				showError(this.getErrorMessage(error, t('whiteboard', 'Failed to delete organization library preset.')))
			} finally {
